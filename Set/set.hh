...
class Set {
 ...
 private:
  ...
  void copy(Node* n, Set<T>& dest) {
    if (n->hasLeftChild()) copy(n->getLeft(), dest);
    dest.insert(n->getData());
    if (n->hasRightChild()) copy(n->getRight(), dest);
  }
 ...
 private:
  void copyIfMember(Node* n, Set<T>& c, Set<T>& dest) {
    if (n->hasLeftChild()) copyIfMember(n->getLeft(), c, dest);
    if (c.member(n->getData())) {
      dest.insert(n->getData());
    }
    if (n->hasRightChild()) copyIfMember(n->getRight(), c, dest);
  }

 public:
  Set<T> Intersect(Set<T>& x) {
    Set<T> result;
 ...
 private:
  ...
  Node* find(T d, Node* n) {
    if (n == nullptr || n->getData() == d) return n;
    if (d < n->getData())
